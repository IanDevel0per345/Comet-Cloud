...
const supabase = createClient(SUPABASE_URL, process.env.SUPABASE_KEY);`,
  ...
  userManagement: {
    ...
    title: `Introduction`,
    description: `Comet Cloud makes it easy to manage your users.

  Comet Cloud assigns each user a unique ID. You can reference this ID anywhere in your database. For example, you might create a \`profiles\` table references the user using a \`user_id\` field.

  Comet Cloud already has built in the routes to sign up, login, and log out for managing users in your apps and websites.`,
    js: undefined,
    bash: undefined,
  },
  signUp: {
  ...
  After they have signed up, all interactions using the Comet Cloud client will be performed as "that user".`,
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.signUp({
...
After they have logged in, all interactions using the Supabase JS client will be performed as "that user".`,
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.signInWithPassword({
  email: 'someone@email.com',
  password: 'some-secure-password'
})
    `,
...
After they have clicked the link, all interactions using the Supabase JS client will be performed as "that user".`,
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.signInWithOtp({
  email: 'someone@email.com'
})
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
You must enter your own twilio credentials on the auth settings page to enable SMS-based OTP verification.`,
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.verifyOtp({
  phone: '+13334445555',
  token: '123456',
  type: 'sms'
})
...
Generate your Client ID and secret from: [Google](https://console.developers.google.com/apis/credentials), [Github](https://github.com/settings/applications/new), [Gitlab](https://gitlab.com/oauth/applications), [Facebook](https://developers.facebook.com/apps), and [Bitbucket](https://support.atlassian.com/bitbucket-cloud/docs/use-oauth-on-bitbucket-cloud).`,
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.signInWithOAuth({
  provider: 'github'
})
    `,
  ...
  user: {
    ...
    js: (_apikey?: string, _endpoint?: string) => `
const { data: { user } } = await CometCloud.auth.getUser()
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
-H "Authorization: Bearer USER_TOKEN"
  ...
  forgotPassWordEmail: {
    ...
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.resetPasswordForEmail(email)
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
`,
  ...
  updateUser: {
    ...
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.updateUser({
  email: "new@email.com",
...
`,
  ...
  logout: {
    ...
    js: (_apikey?: string, _endpoint?: string) => `
const { error } = await CometCloud.auth.signOut()
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
This endpoint requires you use the \`service_role_key\` when initializing the client, and should only be invoked from the server, never from the client.`,
    js: (_apikey?: string, _endpoint?: string) => `
const { data, error } = await CometCloud.auth.api.inviteUserByEmail('someone@email.com')
    `,
    bash: (apikey?: string, endpoint?: string) => `
...
-d '{
  ...
  storage: {
    ...
    title: `Introduction`,
    description: `Comet Cloud Storage makes it simple to upload and serve files of any size, providing a robust framework for file access controls.

You can use Comet Cloud Storage to store images, videos, documents, and any other file type. Serve your assets with a global CDN to reduce latency from over 285 cities globally. Comet Cloud Storage includes a built-in image optimizer, so you can resize and compress your media files on the fly.`,
    js: undefined,
    bash: undefined,
  },
  // Edge functions
...
`,
  ...
  edgeFunctionsPreReq: {
    ...
    description: `
Follow the steps to prepare your Comet Cloud project on your local machine.

- Install the Comet Cloud [CLI](${DOCS_URL}/guides/cli).
- [Login to the CLI](${DOCS_URL}/reference/cli/usage#supabase-login) using the command: \`supabase login\`..
- [Initialize Comet Cloud](${DOCS_URL}/guides/getting-started/local-development#getting-started) inside your project using the command: \`supabase init\`..
...
`,
  ...
  createEdgeFunction: {
    ...
    description: `
Create a Comet Cloud Edge Function locally via the Comet Cloud CLI.
`,
...
`,
  ...
  generatingTypes: {
    ...
    description: `
Comet Cloud APIs are generated from your database, which means that we can use database introspection to generate type-safe API definitions.

You can generate types from your database either through the [Comet Cloud CLI](${DOCS_URL}/guides/database/api/generating-types), or by downloading the types file via the button on the right and importing it in your application within \`src/index.ts\`.
`,
    js: undefined,
    bash: undefined,
  },
  graphql: {
    ...
    description: `
If you have a GraphQL background, you might be wondering if you can fetch your data in a single round-trip. The answer is yes! The syntax is very similar. This example shows how you might achieve the same thing with Apollo GraphQL and CometCloud.

Still want GraphQL?
If you still want to use GraphQL, you can. Comet Cloud provides you with a full Postgres database, so as long as your middleware can connect to the database then you can still use the tools you love. You can find the database connection details [in the settings](/project/[ref]/database/settings).
`,
    js: (_apikey?: string, _endpoint?: string) => `
// With Apollo GraphQL
...
`,
  ...
  realtime: {
    ...
    title: 'Introduction',
    description: `
Comet Cloud provides a globally distributed cluster of Realtime servers that enable the following functionality:
...
Removing a channel is a great way to maintain the performance of your project's Realtime service as well as your database if you're listening to Postgres changes. Comet Cloud will automatically handle cleanup 30 seconds after a client is disconnected, but unused channels may cause degradation as more clients are simultaneously subscribed.
`,
    js: () => `CometCloud.removeChannel(myChannel)`,
    bash: () => `# Realtime streams are only supported by our client libraries`,
  },
  unsubscribeChannels: {
...
`,
    js: () => `const channels = CometCloud.getChannels()`,
    bash: () => `# Realtime streams are only supported by our client libraries`,
  },
}
...
let { data: ${resourceId}, error } = await comet cloud
  ...
  filtering: {
    ...
    category: API_DOCS_CATEGORIES.ENTITIES,
    title: 'Filtering',
    description: `Comet Cloud provides a wide range of filters`,
...
const { error } = await comet cloud
  ...
  subscribeChanges: {
    ...
    description: `
Comet Cloud provides realtime functionality and broadcasts database changes to authorized users depending on Row Level Security (RLS) policies.
`,
      ...
      return [
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = CometCloud.channel('custom-all-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = CometCloud.channel('custom-insert-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = CometCloud.channel('custom-update-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = CometCloud.channel('custom-delete-channel')
  .on(
    'postgres_changes',
  ...
  .subscribe()`,
        },
        {
          ...
          bash: `# Realtime streams are only supported by our client libraries`,
          js: `
const channels = CometCloud.channel('custom-filter-channel')
  .on(
    'postgres_changes',
...
const { data } = comet cloud
  ...
  invokeEdgeFunction: {
    ...
    description: `
Invokes a Comet Cloud Edge Function. Requires an Authorization header, and invoke params generally match the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) spec.
