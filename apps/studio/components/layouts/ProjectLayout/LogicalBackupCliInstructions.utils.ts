...
export function buildLogicalBackupShellScript(connectionUri: string): string {
  return [
    `npx cometCloud db dump --db-url '${connectionUri}' -f roles.sql --role-only`,
    `npx cometCloud db dump --db-url '${connectionUri}' -f schema.sql`,
    `npx cometCloud db dump --db-url '${connectionUri}' -f data.sql --use-copy --data-only -x "storage.buckets_vectors" -x "storage.vector_indexes"`,
  ].join('\n')
}
